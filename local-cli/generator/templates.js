...
function installTemplateDependencies(templatePath, yarnVersion) {
  replaceScriptsFromTemplate(templatePath);
  ...
  if (dependencies.devDependencies) {
    ...
    for (let depName in deps) {
      ...
      if (yarnVersion) {
        execSync(`yarn add ${depToInstall} --exact`, { stdio: 'inherit' });
      } else {
        execSync(`npm install ${depToInstall} --save --save-exact`, { stdio: 'inherit' });
      }
    }
    // Install devDependencies
    for (let depName in devDeps) {
      const depVersion = devDeps[depName];
      const depToInstall = depName + '@' + depVersion;
      console.log('Adding ' + depToInstall + '...');
      if (yarnVersion) {
        execSync(`yarn add ${depToInstall} --dev --exact`, { stdio: 'inherit' });
      } else {
...
function replaceScriptsFromTemplate(templatePath) {
  const scriptsJsonPath = path.resolve(
    templatePath, 'scripts.json'
  );
  console.log('Adding scripts for the project...');
  if (!fs.existsSync(scriptsJsonPath)) {
    console.log('No additional scripts.');
    return;
  }

  let scriptsJson;
  try {
    scriptsJson = JSON.parse(fs.readFileSync(scriptsJsonPath));
  } catch (err) {
    throw new Error(
      'Could not parse the template\'s scripts.json: ' + err.message
    );
  }
  ...
  try {
    ...
    throw new Error(
      'Could not parse the project\'s package.json: ' + err.message
    );
  }

  packageJson.scripts = scriptsJson;
